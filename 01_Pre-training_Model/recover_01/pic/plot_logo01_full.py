...
def parse_log(log_path: Path, kmer: str):
    ...
    # Parse epoch accuracy from checkpoint-saving lines
...
ax.set_title("LOGO full pretraining accuracy across k-mer settings", fontsize=14, pad=10)
ax.legend(frameon=False, ncol=2, fontsize=10)
ax.set_xlim(1, int(epoch_df["epoch"].max()))

ymin = float(epoch_df["accuracy"].min()) - 0.002
ymax = float(epoch_df["accuracy"].max()) + 0.0015
ax.set_ylim(ymin, ymax)

fig.tight_layout()
fig1 = OUTDIR / "logo01_full_accuracy_by_epoch_clean.png"
...
ax.set_ylabel("Average wall time per epoch (seconds)", fontsize=11)
ax.set_title("Average epoch time in our NSCC reproduction", fontsize=14, pad=10)
...
fig.text(
    0.5,
    -0.03,
    "Note: runtime is measured on our NSCC reproduction setup and is not directly equivalent to the paper's hardware setting.",
    ha="center",
    fontsize=9
)

fig.tight_layout()
fig2 = OUTDIR / "logo01_full_avg_time_per_epoch_clean.png"

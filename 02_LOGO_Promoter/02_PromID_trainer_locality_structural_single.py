...
sys.path.append("../")
from bgi.bert4keras.models_locality import build_transformer_model

LOCALITY_MODE = 'single'
...
def train_kfold(train_data_file,
    ...
    for train, test in kfold.split(X, Y):
        ...
        np.savez('./data/kfold_{}_train_and_valid_index_{}_gram_{}_{}_{}.npz'.format(str(k_fold), str(ngram), task_name, RUN_FAMILY, LOCALITY_MODE), train=train,
                 test=test)
        ...
        filename = './data/promoter_best_model_gene_bert_{}_fold_{}_gram_{}_11_Knowledge_structural_structural_single.weights.h5'.format(str(k_fold), str(ngram), task_name)

        modelCheckpoint = ModelCheckpoint(filename, monitor='val_acc', save_best_only=True, save_weights_only=True, verbose=1)
...
if __name__ == '__main__':
    ...
    data_path = './data/5_gram_structural_knowledge'

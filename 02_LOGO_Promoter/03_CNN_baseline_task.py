...
if len(sys.argv) < 2:
    raise SystemExit("Usage: python 03_CNN_baseline_task.py <TASK>, e.g. BOTH / TATA_BOX / NO_TATA_BOX")
...
OUT_FOLD = os.path.join(BASE, f"cnn_{TASK.lower()}_fold_results.csv")
OUT_SUMMARY = os.path.join(BASE, f"cnn_{TASK.lower()}_summary.csv")
...
def build_cnn(vocab_size, input_len):
...
for fold in range(10):
    ...
    model = build_cnn(vocab_size=vocab_size, input_len=input_len)

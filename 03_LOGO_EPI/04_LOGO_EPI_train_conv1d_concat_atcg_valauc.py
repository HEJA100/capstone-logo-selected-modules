...
def train_kfold(CELL, TYPE,
    ...
    for train, test in kfold.split(X, Y):
        ...
        opt_dir = CELL + '/' + TYPE + '/opt_valauc'
        os.makedirs(opt_dir, exist_ok=True)
        filename = opt_dir + '/best_model_gene_bert_{}.h5'.format(str(k_fold))

        modelCheckpoint = ModelCheckpoint(filename, monitor='val_auc', mode='max', save_best_only=True, verbose=1)
...
def evaluate(CELL, TYPE, NUM_ENSEMBL=1, ngram=6, batch_size=128, num_parallel_calls=16, ENHANCER_RESIZED_LEN=2000, PROMOTER_RESIZED_LEN=1000):
    ...
    for t in range(NUM_ENSEMBL):
        ...
        weight_path = CELL + '/' + TYPE + '/opt_valauc/best_model_gene_bert_' + str(t) + '.h5'

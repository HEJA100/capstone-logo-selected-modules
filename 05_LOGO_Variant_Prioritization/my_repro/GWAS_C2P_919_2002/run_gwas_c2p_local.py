...
GWAS_DIR = os.path.join(BASE, "GWAS_C2P")
VCF_PATH = os.path.join(GWAS_DIR, "1000G_GWAS_1408.vcf")
OUTPUT_DIR = VCF_PATH + "_output"
EVO_PATH = os.path.join(OUTPUT_DIR, "1000G_GWAS_1408.vcf.evoall.wholerow")
...
def get_DMatrix_concat_useevo(chrom_df, diff_df, evo_df):
    ...
    print("Chromatin evo feature df shape:", evo_df.shape)
...
def run_one(mark_type):
    ...
    dmat, y_true, test_df_ori = get_DMatrix_concat_useevo(chrom_df, diff_df, evo_df)
    ...
    out_df = test_df_ori.iloc[:, :5].copy()
...
print("\n=== GWAS_C2P local reproduction finished ===")
